...
(   --將用戶`李燕容`新增為教練，並且年資設定為2年
    (select id from "USER" where email = 'lee2000@hexschooltest.io'), 2, '新增教練'
),
(   --將用戶`肌肉棒子`新增為教練，並且年資設定為2年
    (select id from "USER" where email = 'muscle@hexschooltest.io'), 2,'新增教練'
),
...
update "COURSE_BOOKING" set cancelled_at = '2024-11-24 17:00:00',
status = '課程已取消' where user_id = (select id from "USER" where email = 'wXlTq@hexschooltest.io')
...
select "COURSE_BOOKING".user_id, count(*) as total from "COURSE_BOOKING" where "COURSE_BOOKING".user_id = (select id from "USER" where email = 'wXlTq@hexschooltest.io') and status != '課程已取消' 
...
where "COACH_LINK_SKILL".skill_id = (select id from "SKILL" where name = '重訓') order by "COACH".experience_years desc;
